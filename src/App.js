...
// 4.2 Smart Chat Widget
const ChatWidget = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [message, setMessage] = useState("");
  const [chatLog, setChatLog] = useState([
    { sender: 'bot', text: 'Hello! Welcome to TerraSense. Are you looking for information on residential or corporate biophilic design?' }
  ]);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[90] flex flex-col items-end">
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-80 sm:w-96 mb-4 overflow-hidden border border-[#2C4C3B]/10 animate-in slide-in-from-bottom-5 duration-300 flex flex-col h-[400px]">
          <div className="bg-[#2C4C3B] p-4 flex justify-between items-center text-white shrink-0">
            <div className="flex items-center gap-3">
              <div className="relative">
                <img src={logo} alt="TerraSense Icon" className="w-8 h-8 brightness-0 invert opacity-90" />
                <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-[#2C4C3B] rounded-full"></div>
              </div>
              <div>
                <span className="font-bold text-sm block leading-none">TerraSense Team</span>
                <span className="text-xs text-white/70 block mt-1">Usually replies instantly</span>
              </div>
            </div>
          ...
          <div className="flex-1 p-4 bg-[#FAF7F2] overflow-y-auto flex flex-col gap-3">
            ...
            {chatLog.map((chat, idx) => (
              <div key={idx} className={`flex ${chat.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-2xl text-sm shadow-sm ${
                  chat.sender === 'user' 
                    ? 'bg-[#C85A3D] text-white rounded-tr-sm' 
                    : 'bg-white text-[#2C4C3B] border border-[#2C4C3B]/5 rounded-tl-sm'
                }`}>
                  {chat.text}
                </div>
              </div>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-white text-[#2C4C3B] border border-[#2C4C3B]/5 p-3 rounded-2xl rounded-tl-sm shadow-sm flex gap-1 items-center h-10">
                  <div className="w-1.5 h-1.5 bg-[#2C4C3B]/40 rounded-full animate-bounce"></div>
                  <div className="w-1.5 h-1.5 bg-[#2C4C3B]/40 rounded-full animate-bounce delay-100"></div>
                  <div className="w-1.5 h-1.5 bg-[#2C4C3B]/40 rounded-full animate-bounce delay-200"></div>
                </div>
              </div>
            )}
            <div ref={chatEndRef} />
          ...
          <form onSubmit={handleSubmit} className="p-3 bg-white border-t border-[#2C4C3B]/10 flex gap-2 shrink-0">
            <input 
              ...
              className="flex-grow bg-[#FAF7F2] border border-transparent rounded-full px-4 py-2.5 text-sm focus:outline-none focus:bg-white focus:border-[#C85A3D]/30 transition-all"
            />
            <button 
              type="submit" 
              disabled={!message.trim() || isTyping}
              className="bg-[#C85A3D] text-white p-2.5 rounded-full hover:bg-[#a64a32] disabled:opacity-50 disabled:hover:bg-[#C85A3D] transition-colors"
            >
      ...
      <button 
        ...
        className="bg-[#C85A3D] text-white p-4 rounded-full shadow-2xl hover:shadow-[0_10px_25px_rgba(200,90,61,0.5)] hover:scale-105 transition-all duration-300 relative group"
      >
        {!isOpen && (
          <span className="absolute -top-1 -right-1 flex h-4 w-4">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
            <span className="relative inline-flex rounded-full h-4 w-4 bg-[#E8DFD0] border-2 border-[#C85A3D]"></span>
          </span>
        )}
